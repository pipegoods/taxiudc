...
$app->post('/auto', function() use ($app) {
    ...
    verifyRequiredParams(array('name_persona', 'barrio', 'wpp'));
  
    $response = array();
    //capturamos los parametros recibidos y los almacxenamos como un nuevo array
    $param['name_persona']  = $app->request->post('name_persona');
    $param['barrio'] = $app->request->post('barrio');
    $param['wpp'] = $app->request->post('wpp');
    ...
    if ( is_array($param) ) {
        $response["error"] = false;
        $response["message"] = "Taxi creado satisfactoriamente!";
        $response["auto"] = $auto;
    } else {
        $response["error"] = true;
        $response["message"] = "Error al crear Taxi. Por favor intenta nuevamente.";
    }
    echoResponse(201, $response);
});

$app->delete('/auto/:id', function ($id) {
    $db = new DbHandler();
    $response = $db->borrarAuto($id);
